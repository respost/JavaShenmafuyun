...
public class Code_PicUpload_Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
		...
		public void init(ServletConfig config) throws ServletException {
			...
			// 初始化路径
			// 保存文件的目录
			PATH_FOLDER = servletCtx.getRealPath(WebFilePath.thumb+"/");
			// 存放临时文件的目录,存放xxx.tmp文件的目录
			TEMP_FOLDER = servletCtx.getRealPath(WebFilePath.thumb+"/temp/");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// 保存后图片的浏览器访问路径
			String picUrl = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/"+WebFilePath.thumb+"/"+saveName;
			System.out.println("存放目录:" + PATH_FOLDER);
			System.out.println("原文件名:" + filename);
			System.out.println("保存的文件名:" + saveName);
			System.out.println("相对路径："+WebFilePath.thumb+"/"+saveName);
			...
			map.put("path", WebFilePath.thumb+"/"+saveName);
